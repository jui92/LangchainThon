...
# Job Helper Bot (수정본)
#   변경사항(사용자 요청 2,3만 반영):
#   2) "자소서 생성" 후 "질문 생성"을 눌러도 자소서가 화면에서 사라지지 않도록 세션에 보존/항상 표시
#   3) 채점 결과 및 팔로업 채점 결과에서 "수정본 답변(STAR)" 출력 제거 (LLM 프롬프트/파서/표시 모두)
#      → 강점/리스크/개선 포인트 중심 피드백만 남김
#   ※ 그 외 로직(정제/질문/채점/팔로업 흐름)과 UI 구성은 기존 파일 유지
# ################################################################################################
    ...
    for p in paths:
        ...
        for tag in soup.find_all(["h1","h2","h3","h4","p","li"]):
            ...
            if 6 <= len(t) <= 260:
                texts.append(t)
        for t in texts:
            if any(k.lower() in t.lower() for k in ["talent","인재상","인재","people we","who we hire"]):
                out["talent"].append(t)
...
def llm_score_and_coach_strict(clean: Dict, question: str, answer: str, model: str) -> Dict:
    """수정: revised_answer 제거. 총점은 각 기준 합계와 동일하게 강제."""
    hits = retrieve_resume_chunks(question + "\n" + answer[:800], k=4)
    resume_text = "\n".join([f"- {t[:400]}" for _, t in hits])[:1600]
    ctx = json.dumps(clean, ensure_ascii=False)

    # revised_answer 항목 삭제한 스키마
    user_msg = {
        "role":"user",
        "content":(
            f"[회사/직무/채용요건]\n{ctx}\n\n"
...
# 자소서 생성 버튼: 세션 저장(cover_letter_text) → 항상 표시
    ...
    if st.button("새 질문 받기", type="primary"):
        if not st.session_state.clean_struct:
            ...
            if q:
                ...
                st.session_state.answer_text = ""       # 이전 답변만 초기화 (자소서는 건드리지 않음)
...
# ================== 6) 채점 & 코칭 (엄격 모드, revised_answer 제거 반영) ==================
st.header("6) 채점 & 코칭")
